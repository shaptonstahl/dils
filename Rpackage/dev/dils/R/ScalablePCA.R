...
ScalablePCA <- function(x,
  ...
  if( is.logical(subsample) ) {
    ...
    if( is.numeric(subsample) ) {
      if( subsample <= 0 ) {
        stop("subsample must be positive or FALSE")
      }
    } else {
      stop("subsample must be a positive number or FALSE")
    }
  }
  ...
  if( is.numeric(n.subsamples) ) {
    if( n.subsamples <= 0 ) {
      stop("n.subsamples must be positive")
    }
  } else {
    stop("n.subsamples must be a positive number")
  }
  
  if( !missing(ignore.cols) ) {
    ignore.cols <- sort(unique(ignore.cols))
    if( !is.numeric(ignore.cols) ) stop("ignore.cols must be numeric")
  }
  if( !missing(use.cols) ) {
    use.cols <- sort(unique(use.cols))
    if( !is.numeric(use.cols) ) stop("use.cols must be numeric")
  }
  # End Guardians
