...
if(packages.loaded) {
  ...
  con <- url("http://sheer.ucdavis.edu/projects/abstentions/batch/minion_sparse_workunits.RData")
  load(con); close(con)
  ...
  theta[n.legislators] <- 2;      # spike rightmost legislator
    ...
    if( "all" == workunits$votes[project.workunit] ) {
      ...
      minion.output.file.name <- paste("Sparse-long-allvotes-CJR-leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    } else {
      rc <- this.legislature$"compete"$"0.05"$rc
      minion.output.file.name <- paste("Sparse-long-obsvotes-CJR-leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    }
  ...
  theta[n.legislators] <- 2;      # spike rightmost legislator
    ...
    if( "all" == workunits$votes[project.workunit] ) {
      rc <- this.legislature$rc.full
      minion.output.file.name <- paste("Sparse-long-allvotes-RSHcompete-leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    } else {
      rc <- this.legislature$"compete"$"0.05"$rc
      minion.output.file.name <- paste("Sparse-long-obsvotes-RSHcompete-leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    }
