...
if(packages.loaded) {
  ...
  con <- url("http://sheer.ucdavis.edu/projects/abstentions/batch/minion_RSH_2011-08-26_workunits.RData")
  load(con); close(con)
  ...
  if( "full" == workunits$mech[project.workunit] ) {
    ...
    minion.output.file.name <- paste("RSH_2011-08-26_CJR_on_all_votes_leg", 
      workunits$legislature[project.workunit], ".RData", sep="")
  } else {
    ...
    if( "nominate" == workunits$model[project.workunit] ) {
      ...
      minion.output.file.name <- paste("RSH_2011-08-26_NOM_on_", 
      ...
      result <- MCMCirt1d(rc,
        ...
        mcmc=50000)
      result.type <- "MCMCirt1d"
      minion.output.file.name <- paste("RSH_2011-08-26_CJR_on_", 
        workunits$mech[project.workunit], "_at_", 
        workunits$ab.rate[project.workunit], "_leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    } else {  # model is RSH
...
#        check.conv=FALSE, plots=FALSE)
      ...
      result.type <- "RSH"
      minion.output.file.name <- paste("RSH_2011-08-26_RSH_on_", 
        workunits$mech[project.workunit], "_at_", 
        workunits$ab.rate[project.workunit], "_leg", 
        workunits$legislature[project.workunit], ".RData", sep="")
    }
